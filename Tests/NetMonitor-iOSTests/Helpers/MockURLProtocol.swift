...
/// Test-only URLProtocol for iOS unit tests. Intercepts URLSession requests and
/// routes them to a per-session handler closure. Uses a UUID token injected via
/// a custom request header to isolate concurrent test suites from one another.
final class MockURLProtocol: URLProtocol, @unchecked Sendable {
    ...
    /// Creates a URLSession that routes requests to different stubs based on URL
    /// substring matching. Useful for services that make multiple API calls.
