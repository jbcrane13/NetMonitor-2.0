import Testing
import Foundation
@testable import NetMonitor_iOS
import NetMonitorCore

// MARK: - Mock Service

@MainActor
private final class MockNetworkHealthScoreService: NetworkHealthScoreServiceProtocol, @unchecked Sendable {
    var mockScore = NetworkHealthScore(score: 85, grade: "B", latencyMs: 25, packetLoss: 0.02, details: ["latency": "25 ms"])
    var calculateCallCount = 0

    func calculateScore() async -> NetworkHealthScore {
        calculateCallCount += 1
        return mockScore
    }
}

@MainActor
private final class MockPingServiceForHealth: PingServiceProtocol, @unchecked Sendable {
    var mockResults: [PingResult] = []

    func ping(host: String, count: Int, timeout: TimeInterval) async -> AsyncStream<PingResult> {
        let results = mockResults
        return AsyncStream { continuation in
            for r in results { continuation.yield(r) }
            continuation.finish()
        }
    }
    func stop() async {}
    func calculateStatistics(_ results: [PingResult], requestedCount: Int?) async -> PingStatistics? { nil }
}

@MainActor
private final class MockNetworkMonitor: NetworkMonitorServiceProtocol, @unchecked Sendable {
    var isConnected: Bool = true
    var connectionType: ConnectionType = .wifi
    var isExpensive: Bool = false
    var isConstrained: Bool = false
    var statusText: String = "Connected"
    func startMonitoring() {}
    func stopMonitoring() {}
}

// MARK: - Tests

@Suite("NetworkHealthScoreViewModel")
@MainActor
struct NetworkHealthScoreViewModelTests {

    @Test func initialStateIsEmpty() {
        let vm = NetworkHealthScoreViewModel(
            service: MockNetworkHealthScoreService(),
            pingService: MockPingServiceForHealth(),
            networkMonitor: MockNetworkMonitor()
        )
        #expect(vm.currentScore == nil)
        #expect(vm.isCalculating == false)
        #expect(vm.lastUpdated == nil)
        #expect(vm.errorMessage == nil)
    }

    @Test func gradeTextReturnsEmDashWhenNoScore() {
        let vm = NetworkHealthScoreViewModel(
            service: MockNetworkHealthScoreService(),
            pingService: MockPingServiceForHealth(),
            networkMonitor: MockNetworkMonitor()
        )
        #expect(vm.gradeText == "—")
    }

    @Test func scoreValueReturnsZeroWhenNoScore() {
        let vm = NetworkHealthScoreViewModel(
            service: MockNetworkHealthScoreService(),
            pingService: MockPingServiceForHealth(),
            networkMonitor: MockNetworkMonitor()
        )
        #expect(vm.scoreValue == 0)
    }

    @Test func latencyTextReturnsEmDashWhenNoScore() {
        let vm = NetworkHealthScoreViewModel(
            service: MockNetworkHealthScoreService(),
            pingService: MockPingServiceForHealth(),
            networkMonitor: MockNetworkMonitor()
        )
        #expect(vm.latencyText == "—")
    }

    @Test func packetLossTextReturnsEmDashWhenNoScore() {
        let vm = NetworkHealthScoreViewModel(
            service: MockNetworkHealthScoreService(),
            pingService: MockPingServiceForHealth(),
            networkMonitor: MockNetworkMonitor()
        )
        #expect(vm.packetLossText == "—")
    }
}

// MARK: - Scoring Algorithm Tests

@Suite("NetworkHealthScoreService - Algorithm")
struct NetworkHealthScoreAlgorithmTests {

    @Test func perfectScoreWhenAllMetricsExcellent() {
        let score = NetworkHealthScoreService.computeScore(
            latencyMs: 10, packetLoss: 0, dnsMs: 20,
            deviceCount: nil, typicalDeviceCount: nil
        )
        #expect(score == 100)
    }

    @Test func zeroScoreWhenHighLatencyAndLoss() {
        let score = NetworkHealthScoreService.computeScore(
            latencyMs: 500, packetLoss: 0.5, dnsMs: 1000,
            deviceCount: nil, typicalDeviceCount: nil
        )
        #expect(score == 0)
    }

    @Test func gradeAForScoreAbove90() {
        #expect(NetworkHealthScoreService.grade(for: 95) == "A")
        #expect(NetworkHealthScoreService.grade(for: 90) == "A")
    }

    @Test func gradeBForScore80To89() {
        #expect(NetworkHealthScoreService.grade(for: 85) == "B")
        #expect(NetworkHealthScoreService.grade(for: 80) == "B")
    }

    @Test func gradeCForScore70To79() {
        #expect(NetworkHealthScoreService.grade(for: 75) == "C")
    }

    @Test func gradeDForScore60To69() {
        #expect(NetworkHealthScoreService.grade(for: 65) == "D")
    }

    @Test func gradeFBelowSixty() {
        #expect(NetworkHealthScoreService.grade(for: 50) == "F")
        #expect(NetworkHealthScoreService.grade(for: 0) == "F")
    }

    @Test func noDataReturnsZeroScore() {
        let score = NetworkHealthScoreService.computeScore(
            latencyMs: nil, packetLoss: nil, dnsMs: nil,
            deviceCount: nil, typicalDeviceCount: nil
        )
        #expect(score == 0)
    }

    @Test func partialDataComputesNormalizedScore() {
        // Only latency provided — should normalize to latency weight alone
        let score = NetworkHealthScoreService.computeScore(
            latencyMs: 10, packetLoss: nil, dnsMs: nil,
            deviceCount: nil, typicalDeviceCount: nil
        )
        #expect(score == 100) // 35/35 = 100%
    }

    @Test func deviceCountAnomalyReducesScore() {
        let normal = NetworkHealthScoreService.computeScore(
            latencyMs: 10, packetLoss: 0, dnsMs: 20,
            deviceCount: 5, typicalDeviceCount: 5
        )
        let anomaly = NetworkHealthScoreService.computeScore(
            latencyMs: 10, packetLoss: 0, dnsMs: 20,
            deviceCount: 100, typicalDeviceCount: 5
        )
        #expect(normal >= anomaly)
    }
}
